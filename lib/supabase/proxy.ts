...
export async function updateSession(request: NextRequest) {
  ...
  if (!supabaseUrl || !supabaseAnonKey || hasPlaceholderConfig) {
    if (!hasWarnedMissingProxyEnv) {
      ...
      console.warn(
        '[supabase/proxy] Missing Supabase URL or key; skipping session refresh middleware.'
      )
    }
  ...
  try {
    ...
    console.error('[supabase/proxy] Session refresh failed; allowing request through.', error)
  }
