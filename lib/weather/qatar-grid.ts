export interface QatarWeatherGridCell {
  id: string
  latitude: number
  longitude: number
  gridSizeM: number
  row: number
  col: number
}

const GRID_SIZE_M = 5000
const QATAR_REFERENCE_LATITUDE = 25.35
const QATAR_CELL_COUNT = 510

// Natural Earth 10m Qatar boundary, simplified only by coordinate rounding.
// Coordinates are [longitude, latitude].
const QATAR_BOUNDARY: Array<[number, number]> = [
  [50.807872, 24.746649],
  [50.826671, 24.748684],
  [50.852794, 24.770006],
  [50.85963, 24.7956],
  [50.851085, 24.890815],
  [50.842784, 24.916083],
  [50.799978, 24.988756],
  [50.796641, 25.003607],
  [50.807384, 25.031643],
  [50.809744, 25.051215],
  [50.808604, 25.069159],
  [50.802419, 25.076972],
  [50.775727, 25.091864],
  [50.768403, 25.1258],
  [50.766938, 25.162584],
  [50.757823, 25.186225],
  [50.764985, 25.204291],
  [50.763438, 25.225287],
  [50.759044, 25.247504],
  [50.757823, 25.269355],
  [50.765798, 25.323554],
  [50.765147, 25.344468],
  [50.750987, 25.419623],
  [50.762462, 25.462714],
  [50.763845, 25.482408],
  [50.750987, 25.494696],
  [50.778005, 25.527045],
  [50.797093, 25.529896],
  [50.808484, 25.517239],
  [50.811648, 25.498887],
  [50.820508, 25.491293],
  [50.820486, 25.471829],
  [50.847179, 25.467353],
  [50.831309, 25.507799],
  [50.827891, 25.525051],
  [50.826671, 25.543118],
  [50.828624, 25.554348],
  [50.832205, 25.566148],
  [50.832856, 25.578518],
  [50.826671, 25.590888],
  [50.818533, 25.595771],
  [50.79656, 25.599595],
  [50.785004, 25.60456],
  [50.796397, 25.617255],
  [50.809581, 25.620592],
  [50.823497, 25.622138],
  [50.836925, 25.628811],
  [50.84018, 25.634508],
  [50.846528, 25.649848],
  [50.850597, 25.652981],
  [50.857188, 25.650824],
  [50.863536, 25.641425],
  [50.870942, 25.639309],
  [50.87615, 25.631781],
  [50.874278, 25.614936],
  [50.867686, 25.587836],
  [50.867686, 25.567043],
  [50.868663, 25.559556],
  [50.87672, 25.540595],
  [50.881521, 25.535142],
  [50.888682, 25.529486],
  [50.898692, 25.532213],
  [50.900727, 25.534857],
  [50.900076, 25.53974],
  [50.904145, 25.563137],
  [50.904796, 25.587063],
  [50.90919, 25.597724],
  [50.920584, 25.608344],
  [50.927501, 25.60517],
  [50.93572, 25.598334],
  [50.949555, 25.597724],
  [50.955903, 25.604682],
  [50.9699, 25.633857],
  [50.97755, 25.646145],
  [50.94988, 25.636461],
  [50.924327, 25.63231],
  [50.903168, 25.640123],
  [50.888682, 25.666653],
  [50.887543, 25.680894],
  [50.894867, 25.728095],
  [50.897309, 25.732571],
  [50.906993, 25.739936],
  [50.90919, 25.745103],
  [50.90797, 25.749254],
  [50.902843, 25.755276],
  [50.901703, 25.759467],
  [50.901703, 25.789537],
  [50.915538, 25.786444],
  [50.920258, 25.794379],
  [50.923595, 25.805162],
  [50.932872, 25.810614],
  [50.939708, 25.807196],
  [50.945567, 25.798489],
  [50.949229, 25.787014],
  [50.949555, 25.775295],
  [50.963715, 25.783881],
  [50.972992, 25.797512],
  [50.975922, 25.813788],
  [50.970714, 25.830512],
  [50.963878, 25.823676],
  [50.949392, 25.837958],
  [50.951182, 25.85928],
  [50.977306, 25.916449],
  [50.981456, 25.935858],
  [50.984223, 25.98135],
  [50.992524, 25.975735],
  [50.995616, 25.972846],
  [50.998546, 25.967027],
  [51.004731, 25.967027],
  [51.027354, 26.026923],
  [51.047374, 26.053046],
  [51.075938, 26.06391],
  [51.091075, 26.066962],
  [51.133149, 26.082099],
  [51.141856, 26.087795],
  [51.147227, 26.103095],
  [51.170584, 26.124579],
  [51.176117, 26.139594],
  [51.194347, 26.133531],
  [51.214854, 26.13935],
  [51.251231, 26.160102],
  [51.33074, 26.121161],
  [51.346853, 26.104885],
  [51.348481, 26.085639],
  [51.346365, 26.060696],
  [51.347179, 26.039008],
  [51.357677, 26.029731],
  [51.376638, 26.022895],
  [51.382498, 26.006171],
  [51.383474, 25.985582],
  [51.388438, 25.967027],
  [51.403982, 25.954088],
  [51.416515, 25.952094],
  [51.424327, 25.950873],
  [51.46754, 25.954047],
  [51.491954, 25.95189],
  [51.511974, 25.946112],
  [51.52947, 25.937323],
  [51.546072, 25.926093],
  [51.559418, 25.913764],
  [51.570323, 25.898383],
  [51.577485, 25.880845],
  [51.594737, 25.771877],
  [51.594493, 25.758246],
  [51.591563, 25.746649],
  [51.583507, 25.741767],
  [51.577647, 25.745063],
  [51.572032, 25.759508],
  [51.563324, 25.762885],
  [51.555431, 25.758694],
  [51.54835, 25.748847],
  [51.544607, 25.737372],
  [51.546072, 25.728095],
  [51.555024, 25.720649],
  [51.56365, 25.721625],
  [51.570323, 25.727484],
  [51.573985, 25.734931],
  [51.590099, 25.7119],
  [51.576834, 25.688666],
  [51.556814, 25.681627],
  [51.552908, 25.707587],
  [51.539317, 25.703925],
  [51.505138, 25.687161],
  [51.519705, 25.681708],
  [51.534923, 25.673245],
  [51.54713, 25.663153],
  [51.552908, 25.652981],
  [51.54949, 25.631171],
  [51.535899, 25.622382],
  [51.518809, 25.61872],
  [51.505138, 25.612006],
  [51.498302, 25.615912],
  [51.491384, 25.618232],
  [51.493663, 25.565823],
  [51.491547, 25.550645],
  [51.491384, 25.549302],
  [51.48585, 25.540513],
  [51.47755, 25.531073],
  [51.471934, 25.521389],
  [51.474294, 25.512112],
  [51.510997, 25.454291],
  [51.516938, 25.440172],
  [51.518809, 25.426215],
  [51.515717, 25.382839],
  [51.511974, 25.330227],
  [51.513194, 25.308254],
  [51.518809, 25.297065],
  [51.531261, 25.292182],
  [51.590017, 25.283759],
  [51.599132, 25.274888],
  [51.607595, 25.255113],
  [51.611095, 25.237535],
  [51.614594, 25.219794],
  [51.616547, 25.137193],
  [51.605724, 25.034247],
  [51.611013, 25.022366],
  [51.602306, 25.013821],
  [51.586925, 24.953437],
  [51.574067, 24.934394],
  [51.539399, 24.898871],
  [51.528819, 24.874416],
  [51.520844, 24.870592],
  [51.511729, 24.867865],
  [51.505138, 24.864081],
  [51.500255, 24.855373],
  [51.474213, 24.764472],
  [51.47283, 24.759426],
  [51.46754, 24.748684],
  [51.455821, 24.740668],
  [51.450369, 24.721666],
  [51.443614, 24.679755],
  [51.430431, 24.664496],
  [51.389985, 24.642483],
  [51.381521, 24.635077],
  [51.37615, 24.611884],
  [51.363536, 24.59634],
  [51.349783, 24.583808],
  [51.340587, 24.569892],
  [51.328949, 24.584784],
  [51.332693, 24.598863],
  [51.341645, 24.614691],
  [51.346853, 24.635077],
  [51.339122, 24.646186],
  [51.321137, 24.648179],
  [51.285981, 24.645006],
  [51.284434, 24.662502],
  [51.278087, 24.662991],
  [51.267914, 24.656195],
  [51.254568, 24.651801],
  [51.23699, 24.650051],
  [51.220958, 24.644232],
  [51.212576, 24.633694],
  [51.214854, 24.627183],
  [51.215258, 24.62585],
  [51.147451, 24.576279],
  [51.095465, 24.560182],
  [51.038879, 24.559872],
  [50.978883, 24.567726],
  [50.928602, 24.587208],
  [50.881008, 24.636404]
]

function degreesPerMeterLatitude() {
  return 1 / 111_320
}

function degreesPerMeterLongitude(latitude: number) {
  return 1 / (111_320 * Math.cos((latitude * Math.PI) / 180))
}

function isInsidePolygon(longitude: number, latitude: number) {
  let inside = false
  let previousIndex = QATAR_BOUNDARY.length - 1

  for (let index = 0; index < QATAR_BOUNDARY.length; index += 1) {
    const [currentLon, currentLat] = QATAR_BOUNDARY[index]
    const [previousLon, previousLat] = QATAR_BOUNDARY[previousIndex]
    const crossesLatitude = currentLat > latitude !== previousLat > latitude
    const intersectionLon =
      ((previousLon - currentLon) * (latitude - currentLat)) / (previousLat - currentLat) + currentLon

    if (crossesLatitude && longitude < intersectionLon) {
      inside = !inside
    }

    previousIndex = index
  }

  return inside
}

function getBounds() {
  return QATAR_BOUNDARY.reduce(
    (bounds, [longitude, latitude]) => ({
      minLon: Math.min(bounds.minLon, longitude),
      minLat: Math.min(bounds.minLat, latitude),
      maxLon: Math.max(bounds.maxLon, longitude),
      maxLat: Math.max(bounds.maxLat, latitude),
    }),
    {
      minLon: Number.POSITIVE_INFINITY,
      minLat: Number.POSITIVE_INFINITY,
      maxLon: Number.NEGATIVE_INFINITY,
      maxLat: Number.NEGATIVE_INFINITY,
    }
  )
}

function polygonVerticesInsideCell(longitude: number, latitude: number, lonStep: number, latStep: number) {
  const minLon = longitude - lonStep / 2
  const maxLon = longitude + lonStep / 2
  const minLat = latitude - latStep / 2
  const maxLat = latitude + latStep / 2

  return QATAR_BOUNDARY.filter(
    ([vertexLon, vertexLat]) => vertexLon >= minLon && vertexLon <= maxLon && vertexLat >= minLat && vertexLat <= maxLat
  )
}

function getCellRepresentativePoint(longitude: number, latitude: number, lonStep: number, latStep: number) {
  if (isInsidePolygon(longitude, latitude)) {
    return { longitude, latitude }
  }

  const corners = [
    { longitude: longitude - lonStep / 2, latitude: latitude - latStep / 2 },
    { longitude: longitude + lonStep / 2, latitude: latitude - latStep / 2 },
    { longitude: longitude + lonStep / 2, latitude: latitude + latStep / 2 },
    { longitude: longitude - lonStep / 2, latitude: latitude + latStep / 2 },
  ].filter((corner) => isInsidePolygon(corner.longitude, corner.latitude))

  if (corners.length > 0) {
    return {
      longitude: corners.reduce((sum, corner) => sum + corner.longitude, 0) / corners.length,
      latitude: corners.reduce((sum, corner) => sum + corner.latitude, 0) / corners.length,
    }
  }

  const vertices = polygonVerticesInsideCell(longitude, latitude, lonStep, latStep)
  if (vertices.length > 0) {
    return {
      longitude: vertices.reduce((sum, [vertexLon]) => sum + vertexLon, 0) / vertices.length,
      latitude: vertices.reduce((sum, [, vertexLat]) => sum + vertexLat, 0) / vertices.length,
    }
  }

  return { longitude, latitude }
}

function cellIntersectsQatar(longitude: number, latitude: number, lonStep: number, latStep: number) {
  if (isInsidePolygon(longitude, latitude)) return true

  const corners = [
    [longitude - lonStep / 2, latitude - latStep / 2],
    [longitude + lonStep / 2, latitude - latStep / 2],
    [longitude + lonStep / 2, latitude + latStep / 2],
    [longitude - lonStep / 2, latitude + latStep / 2],
  ] as Array<[number, number]>

  if (corners.some(([cornerLon, cornerLat]) => isInsidePolygon(cornerLon, cornerLat))) return true
  return polygonVerticesInsideCell(longitude, latitude, lonStep, latStep).length > 0
}

export function generateQatarWeatherGrid(): QatarWeatherGridCell[] {
  const latStep = GRID_SIZE_M * degreesPerMeterLatitude()
  const lonStep = GRID_SIZE_M * degreesPerMeterLongitude(QATAR_REFERENCE_LATITUDE)
  const bounds = getBounds()
  const cells: QatarWeatherGridCell[] = []

  let row = 0
  for (
    let latitude = Math.floor(bounds.minLat / latStep) * latStep;
    latitude <= bounds.maxLat;
    latitude += latStep
  ) {
    let col = 0
    for (
      let longitude = Math.floor(bounds.minLon / lonStep) * lonStep;
      longitude <= bounds.maxLon;
      longitude += lonStep
    ) {
      if (cellIntersectsQatar(longitude, latitude, lonStep, latStep)) {
        const representative = getCellRepresentativePoint(longitude, latitude, lonStep, latStep)
        cells.push({
          id: `W5_${String(cells.length + 1).padStart(3, '0')}`,
          latitude: Number(representative.latitude.toFixed(6)),
          longitude: Number(representative.longitude.toFixed(6)),
          gridSizeM: GRID_SIZE_M,
          row,
          col,
        })
      }
      col += 1
    }
    row += 1
  }

  return cells.slice(0, QATAR_CELL_COUNT)
}

export const QATAR_WEATHER_GRID_CELL_COUNT = QATAR_CELL_COUNT
